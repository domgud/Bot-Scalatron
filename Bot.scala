...
/**
 *  Modifications:
 *      - added collector bot (uses DFS to get back to master)
 *      - added landmine bot
 */
...
object ControlFunction
{
    ...
    def forMaster(bot: Bot) {
        ...
        //moved some stuff to another method to be easier to understand
        moveBot(bot, directionValue)

        if (bot.energy > 1000) {
            if (random.nextDouble() > 0.10) { //90% chance to spawn a collector
                spawnCollector(bot)
            } else {
                spawnLandmine(bot)
            }
